 ...
 * ===========================================================
 * Tensor-product space-time code with Q^s finite elements in space and dG-Q^r
 * finite elements in time: cG(s)dG(r) Using Gauss-Legendre quadrature for the
 * temporal quadrature points, since it was shown in [RothThieleKöcherWick2022]
 * that this yields better temporal error estimates The time interval I = (0,T)
 * is being divided into subintervals I_1, I_2, ..., I_M and then instead of
 * solving the PDE all at once on the full space-time domain Q := I x Ω we solve
 * sequentially (forward-in-time) on time slabs Q_n := I_n x Ω to reduce the
 * size of the linear systems. This also allows the usage of different temporal
 * polynomial degrees on each slab, i.e. r := (r_1, r_2, ..., r_M) is now a
 * multiindex.
 *
 * Author: Julian Roth, 2022
 *
 * For more information on tensor-product space-time finite elements please also
 * check out the DTM-project by Uwe Köcher and contributors.
 */
...
Tensor<1, dim> Solution<dim>::gradient(const Point<dim> &p,
  ...
  switch (dim) {
    case 2: {
      ...
      grad[0] =
          M_PI * std::cos(M_PI * p[0]) * (1 + p[1]) * std::exp(-0.5 * p[1]);
      grad[1] =
          0.5 * std::sin(M_PI * p[0]) * (1 - p[1]) * std::exp(-0.5 * p[1]);
      return grad;
    }
    case 3: {
      // z(t,x,y) := 1 + (x - x_0(t))^2 + (y - y_0(t))^2 with x_0, y_0 as
      // defined in Solution<dim>::value ∂_x u(t,x,y) = 2(x_0(t)-x) / z(t,x,y)^2
...
double RightHandSide<dim>::value(const Point<dim> &p,
  ...
  switch (dim) {
    ...
    case 2: {
      ...
      // f has been computed symbolically with the Python package SymPy and the
      // following code has been written by its code generation feature
      // (https://docs.sympy.org/latest/modules/codegen.html)
      return 128 *
             (-8 * std::pow(-4 * p[0] + std::cos(2 * M_PI * t) + 2, 2) -
              8 * std::pow(-4 * p[1] + std::sin(2 * M_PI * t) + 2, 2) +
              M_PI *
                  (-2 * p[0] * std::sin(2 * M_PI * t) +
                   2 * p[1] * std::cos(2 * M_PI * t) -
                   M_SQRT2 * std::cos(M_PI * (2 * t + 1.0 / 4.0))) *
                  (std::pow(-4 * p[0] + std::cos(2 * M_PI * t) + 2, 2) +
                   std::pow(-4 * p[1] + std::sin(2 * M_PI * t) + 2, 2) + 16) +
              128) /
             std::pow(std::pow(-4 * p[0] + std::cos(2 * M_PI * t) + 2, 2) +
                          std::pow(-4 * p[1] + std::sin(2 * M_PI * t) + 2, 2) +
                          16,
                      3);
    }
    default: {
...
Slab::Slab(unsigned int r, double start_time, double end_time)
    : time_fe(QGauss<1>(r + 1)),
      time_dof_handler(time_triangulation),
...
class SpaceTime {
 public:
  SpaceTime(unsigned int s, std::vector<unsigned int> r,
            std::vector<double> time_points = {0., 1.},
            unsigned int max_n_refinement_cycles = 3, bool refine_space = true,
 ...
 private:
  ...
  void output_svg(std::ofstream &out, Vector<double> &space_solution,
                  double time_point, double x_min, double x_max, double y_min,
                  double y_max) const;  // only for 1D in space
  void process_solution(std::shared_ptr<Slab> &slab, const unsigned int cycle,
                        bool last_slab);
  ...
  double start_time, end_time;
  std::set<std::pair<double, unsigned int> >
      time_support_points;  // (time_support_point, support_point_index)
...
template <int dim>
SpaceTime<dim>::SpaceTime(unsigned int s, std::vector<unsigned int> r,
                          std::vector<double> time_points,
                          unsigned int max_n_refinement_cycles,
                          bool refine_space, bool refine_time)
    : space_fe(s),
  ...
  // r = [r_1, r_2, ..., r_M] with r_k is the temporal FE degree on I_k =
  // (t_{k-1},t_k]
  ...
  for (unsigned int k = 0; k < r.size(); ++k)
    slabs.push_back(
        std::make_shared<Slab>(r[k], time_points[k], time_points[k + 1]));

  start_time = time_points[0];
  end_time = time_points[time_points.size() - 1];
}

void print_1d_grid(std::ofstream &out, Triangulation<1> &triangulation,
                   double start, double end) {
  out << "<svg width='1200' height='200' xmlns='http://www.w3.org/2000/svg' "
         "version='1.1'>"
      << std::endl;
  out << "<rect fill='white' width='1200' height='200'/>" << std::endl;
  out << "  <line x1='100' y1='100' x2='1100' y2='100' "
         "style='stroke:black;stroke-width:4'/>"
      << std::endl;  // timeline
  out << "  <line x1='100' y1='125' x2='100' y2='75' "
         "style='stroke:black;stroke-width:4'/>"
      << std::endl;  // first tick
  ...
  for (auto &cell : triangulation.active_cell_iterators())
    out << "  <line x1='"
        ...
        << "' y1='125' x2='"
        << (int)(1000 * (cell->vertex(1)[0] - start) / (end - start)) + 100
        << "' y2='75' style='stroke:black;stroke-width:4'/>" << std::endl;
...
void print_1d_grid_slabwise(std::ofstream &out,
  ...
  for (unsigned int k = 0; k < slabs.size(); ++k)
    out << "  <line x1='"
        ...
        << "' y2='100' style='stroke:" << ((k % 2) ? "blue" : "black")
        << ";stroke-width:4'/>" << std::endl;  // timeline

  // out << "  <line x1='100' y1='100' x2='1100' y2='100'
  // style='stroke:black;stroke-width:4'/>" << std::endl; // timeline
  out << "  <line x1='100' y1='125' x2='100' y2='75' "
         "style='stroke:black;stroke-width:4'/>"
      << std::endl;  // first tick
...
template <>
void SpaceTime<2>::print_grids(std::string file_name_space,
...
template <>
void SpaceTime<1>::print_grids(std::string file_name_space,
                               std::string file_name_time) {
  // space
  std::ofstream out_space(file_name_space);
...
void SpaceTime<1>::make_grids() {
  // create grids
  GridGenerator::hyper_rectangle(space_triangulation, Point<1>(0.),
                                 Point<1>(1.));
  for (auto &slab : slabs)
    GridGenerator::hyper_rectangle(slab->time_triangulation,
  ...
  for (auto &cell : space_triangulation.cell_iterators())
    for (unsigned int face = 0; face < GeometryInfo<1>::faces_per_cell; face++)
      if (cell->face(face)->at_boundary()) cell->face(face)->set_boundary_id(0);

  // globally refine the grids
  space_triangulation.refine_global(2);
  for (auto &slab : slabs) slab->time_triangulation.refine_global(0);
}
...
void SpaceTime<dim>::setup_system(std::shared_ptr<Slab> &slab, unsigned int k) {
  std::cout << "Slab Q_" << k << " = Ω x (" << slab->start_time << ","
            << slab->end_time << "):" << std::endl;
  std::cout << "   Finite Elements: cG(" << space_fe.get_degree()
            << ") (space), dG(" << slab->time_fe.get_degree() << ") (time)"
            << std::endl;
  std::cout << "   Number of active cells: "
            << space_triangulation.n_active_cells() << " (space), "
            << slab->time_triangulation.n_active_cells() << " (time)"
            << std::endl;
  slab->time_dof_handler.distribute_dofs(slab->time_fe);
  std::cout << "   Number of degrees of freedom: " << space_dof_handler.n_dofs()
            << " (space), " << slab->time_dof_handler.n_dofs() << " (time)"
            << std::endl;

  /////////////////////////////////////////////////////////////////////////////////////////
  // space-time sparsity pattern = tensor product of spatial and temporal
  // sparsity pattern
  //
  ...
  // for Gauss-Legendre quadrature we need to couple all temporal DoFs between
  ...
  DynamicSparsityPattern dsp(space_dof_handler.n_dofs() *
                             slab->time_dof_handler.n_dofs());
  for (auto &space_entry : space_dsp)
    for (auto &time_entry : time_dsp)
      dsp.add(space_entry.row() + space_dof_handler.n_dofs() *
                                      time_entry.row(),  // test  function
              space_entry.column() + space_dof_handler.n_dofs() *
                                         time_entry.column()  // trial function
      );
  ...
  system_rhs.reinit(space_dof_handler.n_dofs() *
                    slab->time_dof_handler.n_dofs());
}
...
void SpaceTime<dim>::assemble_system(std::shared_ptr<Slab> &slab) {
  ...
  std::vector<types::global_dof_index> space_local_dof_indices(
      space_dofs_per_cell);
  ...
  std::vector<types::global_dof_index> time_local_dof_indices(
      time_dofs_per_cell);
  std::vector<types::global_dof_index> time_prev_local_dof_indices(
      time_dofs_per_cell);

  // time FEValues for t_m^+ on current time interval I_m
  FEValues<1> time_fe_face_values(
      slab->time_fe, Quadrature<1>({Point<1>(0.)}),
      update_values);  // using left box rule quadrature
  // time FEValues for t_m^- on previous time interval I_{m-1}
  FEValues<1> time_prev_fe_face_values(
      slab->time_fe, Quadrature<1>({Point<1>(1.)}),
      update_values);  // using right box rule quadrature

  // local contributions on space-time cell
  FullMatrix<double> cell_matrix(space_dofs_per_cell * time_dofs_per_cell,
                                 space_dofs_per_cell * time_dofs_per_cell);
  FullMatrix<double> cell_jump(space_dofs_per_cell * time_dofs_per_cell,
                               space_dofs_per_cell * time_dofs_per_cell);
  Vector<double> cell_rhs(space_dofs_per_cell * time_dofs_per_cell);
  std::vector<types::global_dof_index> local_dof_indices(space_dofs_per_cell *
                                                         time_dofs_per_cell);
  ...
  for (const auto &space_cell : space_dof_handler.active_cell_iterators()) {
    ...
    for (const auto &time_cell :
         slab->time_dof_handler.active_cell_iterators()) {
      ...
      for (const unsigned int qq : time_fe_values.quadrature_point_indices()) {
        ...
        for (const unsigned int q :
             space_fe_values.quadrature_point_indices()) {
          ...
          for (const unsigned int i : space_fe_values.dof_indices())
            for (const unsigned int ii : time_fe_values.dof_indices()) {
              ...
              cell_rhs(i + ii * space_dofs_per_cell) +=
                  (space_fe_values.shape_value(i, q) *
                   time_fe_values.shape_value(ii, qq) *  // ϕ_{i,ii}(t_qq, x_q)
              ...
              for (const unsigned int j : space_fe_values.dof_indices())
                for (const unsigned int jj : time_fe_values.dof_indices())
                  cell_matrix(j + jj * space_dofs_per_cell,
                              i + ii * space_dofs_per_cell) +=
                      (space_fe_values.shape_value(i, q) *
                           time_fe_values.shape_grad(
                               ii, qq)[0] *  // ∂_t ϕ_{i,ii}(t_qq, x_q)
                           space_fe_values.shape_value(j, q) *
                           time_fe_values.shape_value(
                               jj, qq)  //     ϕ_{j,jj}(t_qq, x_q)
                                        // +
                       + space_fe_values.shape_grad(i, q) *
                             time_fe_values.shape_value(
                                 ii, qq) *  // ∇_x ϕ_{i,ii}(t_qq, x_q)
                             space_fe_values.shape_grad(j, q) *
                             time_fe_values.shape_value(
                                 jj, qq)  // ∇_x ϕ_{j,jj}(t_qq, x_q)
                       ) *
                      space_fe_values.JxW(q) *
                      time_fe_values.JxW(qq);  // d(t,x)
            }
        }
      }

      // assemble jump terms in system matrix and intial condition in RHS
      // jump terms: ([u]_m,φ_m^+)_Ω = (u_m^+,φ_m^+)_Ω - (u_m^-,φ_m^+)_Ω = (A) -
      // (B)
            ...
            for (const unsigned int j : space_fe_values.dof_indices())
              for (const unsigned int jj : time_fe_values.dof_indices())
                cell_matrix(j + jj * space_dofs_per_cell,
                            i + ii * space_dofs_per_cell) +=
                    (space_fe_values.shape_value(i, q) *
                     time_fe_face_values.shape_value(
                         ii, 0) *  //  ϕ_{i,ii}(t_m^+, x_q)
                     space_fe_values.shape_value(j, q) *
                     time_fe_face_values.shape_value(
                         jj, 0)  //  ϕ_{j,jj}(t_m^+, x_q)
                     ) *
      ...
      if (time_cell->active_cell_index() == 0) {
        ...
        for (const unsigned int q :
          ...
          for (const unsigned int j : space_fe_values.dof_indices()) {
            initial_solution_x_q +=
                initial_solution[space_local_dof_indices[j]] *
                space_fe_values.shape_value(j, q);
          }
            ...
            for (const unsigned int ii : time_fe_values.dof_indices()) {
              cell_rhs(i + ii * space_dofs_per_cell) +=
                  (initial_solution_x_q *  // u0(x_q)
                   space_fe_values.shape_value(i, q) *
                   time_fe_face_values.shape_value(ii,
                                                   0) *  // ϕ_{i,ii}(0^+, x_q)
                   space_fe_values.JxW(q)                // d(x)
                  );
            }
        ...
        // NOTE: cell_jump is a space-time cell matrix because we are using
        // Gauss-Legendre quadrature in time
        for (const unsigned int q : space_fe_values.quadrature_point_indices())
          for (const unsigned int i : space_fe_values.dof_indices())
            for (const unsigned int ii : time_fe_values.dof_indices())
              for (const unsigned int j : space_fe_values.dof_indices())
                for (const unsigned int jj : time_fe_values.dof_indices())
                  cell_jump(j + jj * space_dofs_per_cell,
                            i + ii * space_dofs_per_cell) +=
                      (-1. * space_fe_values.shape_value(i, q) *
                       time_prev_fe_face_values.shape_value(
                           ii, 0) *  // -ϕ_{i,ii}(t_m^-, x_q)
                       space_fe_values.shape_value(j, q) *
                       time_fe_face_values.shape_value(
                           jj, 0)  //  ϕ_{j,jj}(t_m^+, x_q)
      ...
      for (const unsigned int i : space_fe_values.dof_indices())
        for (const unsigned int ii : time_fe_values.dof_indices()) {
          // right hand side
          system_rhs(space_local_dof_indices[i] +
                     time_local_dof_indices[ii] * space_dof_handler.n_dofs()) +=
              cell_rhs(i + ii * space_dofs_per_cell);
          ...
          for (const unsigned int j : space_fe_values.dof_indices())
            for (const unsigned int jj : time_fe_values.dof_indices())
              system_matrix.add(
                  space_local_dof_indices[i] +
                      time_local_dof_indices[ii] * space_dof_handler.n_dofs(),
                  space_local_dof_indices[j] +
                      time_local_dof_indices[jj] * space_dof_handler.n_dofs(),
                  cell_matrix(i + ii * space_dofs_per_cell,
              ...
              for (const unsigned int jj : time_fe_values.dof_indices())
                system_matrix.add(
                    space_local_dof_indices[i] +
                        time_local_dof_indices[ii] * space_dof_handler.n_dofs(),
                    space_local_dof_indices[j] +
                        time_prev_local_dof_indices[jj] *
                            space_dof_handler.n_dofs(),
                    cell_jump(i + ii * space_dofs_per_cell,
                              j + jj * space_dofs_per_cell));

      // prepare next time cell
      if (time_cell->active_cell_index() <
          slab->time_triangulation.n_active_cells() - 1) {
...
void SpaceTime<dim>::apply_boundary_conditions(std::shared_ptr<Slab> &slab) {
  ...
  FEValues<1> time_fe_values(
      slab->time_fe, Quadrature<1>(slab->time_fe.get_unit_support_points()),
      update_quadrature_points);
  std::vector<types::global_dof_index> time_local_dof_indices(
      slab->time_fe.n_dofs_per_cell());
  ...
  for (const auto &time_cell : slab->time_dof_handler.active_cell_iterators()) {
    ...
    for (const unsigned int qq : time_fe_values.quadrature_point_indices()) {
      ...
      time_support_points.insert(
          std::make_pair(t_qq, time_local_dof_indices[qq]));

      // determine spatial boundary values at temporal support point
      std::map<types::global_dof_index, double> boundary_values;
      VectorTools::interpolate_boundary_values(space_dof_handler, 0,
                                               solution_func, boundary_values);

      // calculate the correct space-time entry and apply the Dirichlet BC
      for (auto &entry : boundary_values) {
        types::global_dof_index id =
            entry.first +
            time_local_dof_indices[qq] * space_dof_handler.n_dofs();

        // apply BC
        for (typename SparseMatrix<double>::iterator p =
                 system_matrix.begin(id);
             p != system_matrix.end(id); ++p)
          p->value() = 0.;
...
// To avoid using external plotting software, we create the 1D output in the
// form of an SVG file ourselves. cG(1):            we use SVG's "line" to
// visualize solution cG(2), cG(3):     we use SVG's "path" to visualize
// solution with quadratic/cubic Bezier curves cG(s) with s > 3: we use a
// sufficiently high number of support points in the spatial cell and
// interpolate linearly
template <>
void SpaceTime<1>::output_svg(std::ofstream &out,
                              Vector<double> &space_solution, double time_point,
  ...
  const unsigned int n_points_per_cell =
      (space_fe.get_degree() <= 3) ? 2 : std::pow(2, space_fe.get_degree()) + 4;
  QIterated<1> space_quad(
      QTrapez<1>(),
      n_points_per_cell - 1);  // need to use QTrapezoid for deal.II 9.3.0
  FEValues<1> space_fe_values(
      space_fe, space_quad,
      update_values | update_gradients | update_quadrature_points);

  // create 2D vectors to store the quadrature points and the solution at these
  // points
  std::vector<std::vector<Point<1> > > q_points(
      space_triangulation.n_active_cells(),
      std::vector<Point<1> >(n_points_per_cell, Point<1>(0.)));
  std::vector<std::vector<double> > u_values(
      space_triangulation.n_active_cells(),
      std::vector<double>(n_points_per_cell));
  // for the quadratic and cubic Bezier curves, we also need the gradients at
  // the support points
  std::vector<std::vector<Tensor<1, 1> > > u_gradients(
      space_triangulation.n_active_cells(),
      std::vector<Tensor<1, 1> >(n_points_per_cell));
  ...
  // print out the spatial grid with ticks for the edges of the spatial cells
  // and the solution
  out << "<svg width='1200' height='1200' xmlns='http://www.w3.org/2000/svg' "
         "version='1.1'>"
      << std::endl;
  out << "<rect fill='white' width='1200' height='1200'/>" << std::endl;
  out << "  <text x='500' y='50' font-size='4em'>t = " << time_point
      << "</text>" << std::endl;  // title
  out << "  <line x1='50' y1='" << y_coord(0.) << "' x2='1150' y2='"
      << y_coord(0.) << "' style='stroke:black;stroke-width:4'/>"
      << std::endl;  // x-axis (horizontal)
  out << "  <line x1='50' y1='50' x2='50' y2='1150' "
         "style='stroke:black;stroke-width:4'/>"
      << std::endl;  // y-axis (vertical)

  // ticks
  for (auto &space_cell : space_triangulation.active_cell_iterators())
    out << "  <line x1='" << x_coord(space_cell->vertex(1)[0]) << "' y1='"
        << y_coord(0.) + 25 << "' x2='" << x_coord(space_cell->vertex(1)[0])
        << "' y2='" << y_coord(0.) - 25
        << "' style='stroke:black;stroke-width:4'/>" << std::endl;

  // arrow of x-axis
  out << "  <line x1='1125' y1='" << y_coord(0.) + 25 << "' x2='1150' y2='"
      << y_coord(0.) << "' style='stroke:black;stroke-width:4'/>" << std::endl;
  out << "  <line x1='1125' y1='" << y_coord(0.) - 25 << "' x2='1150' y2='"
      << y_coord(0.) << "' style='stroke:black;stroke-width:4'/>" << std::endl;

  // arrow of y-axis
  out << "  <line x1='75' y1='75' x2='50' y2='50' "
         "style='stroke:black;stroke-width:4'/>"
      << std::endl;
  out << "  <line x1='25' y1='75' x2='50' y2='50' "
         "style='stroke:black;stroke-width:4'/>"
      << std::endl
      << std::endl;
  ...
  if (space_fe.get_degree() == 2) {
    ...
    for (const auto &space_cell : space_dof_handler.active_cell_iterators()) {
      unsigned int i = space_cell->index();
      // to construct a quadratic Bezier curve of a parabola p on the interval
      // [a,b] we need: 1.) the value at the left boundary:      p(a) 2.) the
      // value at the right boundary:     p(b) 3.) the gradient at the left
      // boundary:  p'(a) 4.) the gradient at the right boundary: p'(b)
      //
      // we then have the following three control points for the Bezier curve:
      // 1.) left boundary: (a, p(a))
      // 2.) right boundary: (b, p(b))
      // 3.) the intersection of the tangents to the parabola at the left and
      // right boundary: (c, p'(a)*(c-a)+p(a)) solving for c we then get c =
      // [p(a)-p(b)-a*p'(a)+b*p'(b)] / [p'(b)-p'(a)] = (a+b)/2
      ...
      out << "  <path d='M" << x_coord(a) << "," << y_coord(val_a) << " Q"
          << x_coord(c) << "," << y_coord(val_c) << " " << x_coord(b) << ","
          << y_coord(val_b)
          << "' style='stroke:blue;stroke-width:2' fill='transparent'/>"
          << std::endl;
    }
    ...
    for (const auto &space_cell : space_dof_handler.active_cell_iterators()) {
      unsigned int i = space_cell->index();
      // to construct a cubic Bezier curve of a parabola p on the interval [a,b]
      // we need: 1.) the value at the left boundary:      p(a) 2.) the value at
      // the right boundary:     p(b) 3.) the gradient at the left boundary:
      // p'(a) 4.) the gradient at the right boundary: p'(b)
      //
      // link on cubic Bezier interpolation:
      ...
      // 1.) left boundary: (a, p(a))
      // 2.) right boundary: (b, p(b))
      // 3.) a point on the tangent to the left boundary: (c, p'(a)*(c-a)+p(a))
      // with c = (2a+b)/3 4.) a point on the tangent to the right boundary: (d,
      // p'(b)*(d-b)+p(b)) with b = (a+2b)/3
      double a = q_points[i][0][0];
      double b = q_points[i][1][0];
      double val_a = u_values[i][0];
      double val_b = u_values[i][1];
      ...
      out << "  <path d='M" << x_coord(a) << "," << y_coord(val_a) << " C"
          << x_coord(c) << "," << y_coord(val_c) << " " << x_coord(d) << ","
          << y_coord(val_d) << " " << x_coord(b) << "," << y_coord(val_b)
          << "' style='stroke:blue;stroke-width:2' fill='transparent'/>"
          << std::endl;
    }
  } else {
    ...
    for (unsigned int i = 0; i < space_triangulation.n_active_cells(); ++i)
      for (unsigned int j = 0; j < n_points_per_cell - 1; ++j)
        out << "  <line x1='" << x_coord(q_points[i][j][0]) << "' y1='"
            << y_coord(u_values[i][j]) << "' x2='"
            << x_coord(q_points[i][j + 1][0]) << "' y2='"
            << y_coord(u_values[i][j + 1])
            << "' style='stroke:blue;stroke-width:2'/>" << std::endl;
  }
...
void SpaceTime<1>::output_results(const unsigned int refinement_cycle) {
  ...
  std::string output_dir =
      "output/dim=1/cycle=" + std::to_string(refinement_cycle) + "/";
  for (auto dir : {"output/", "output/dim=1/", output_dir.c_str()})
    mkdir(dir, S_IRWXU);

  // highest and lowest value of solution (hardcoded for the given 1D problem)
  double y_max = 1.4;  // std::max(*std::max_element(solution.begin(),
                       // solution.end()), 0.);
  double y_min = 0.0;  // std::min(*std::min_element(solution.begin(),
                       // solution.end()), 0.);
  ...
  for (auto time_point : time_support_points) {
    ...
    std::ofstream output(output_dir + "solution" +
                         Utilities::int_to_string(n_snapshots, 5) + ".svg");
...
void SpaceTime<2>::output_results(const unsigned int refinement_cycle) {
  // create output directory if necessary
  std::string output_dir =
      "output/dim=2/cycle=" + std::to_string(refinement_cycle) + "/";
  ...
  for (auto time_point : time_support_points) {
    ...
    std::ofstream output(output_dir + "solution" +
                         Utilities::int_to_string(n_snapshots, 5) + ".vtk");
...
template <int dim>
void SpaceTime<dim>::process_solution(std::shared_ptr<Slab> &slab,
                                      const unsigned int cycle,
                                      bool last_slab) {
  Solution<dim> solution_func;

  FEValues<1> time_fe_values(
      slab->time_fe, QGauss<1>(slab->time_fe.degree + 2),
  ...
  for (const auto &time_cell : slab->time_dof_handler.active_cell_iterators()) {
    ...
    for (const unsigned int qq : time_fe_values.quadrature_point_indices()) {
      ...
      for (const unsigned int ii : time_fe_values.dof_indices()) {
        for (unsigned int i = 0; i < space_dof_handler.n_dofs(); ++i)
          space_solution(i) += solution(i + time_local_dof_indices[ii] *
                                                space_dof_handler.n_dofs()) *
      ...
      VectorTools::integrate_difference(
          space_dof_handler, space_solution, solution_func, difference_per_cell,
          QGauss<dim>(space_fe.degree + 2), VectorTools::L2_norm);
      const double L2_error_t_qq = VectorTools::compute_global_error(
  ...
  if (last_slab) {
    ...
    const unsigned int n_active_cells =
        space_triangulation.n_active_cells() * n_active_time_cells;
...
void SpaceTime<dim>::run() {
  ...
  for (unsigned int cycle = 0; cycle < max_n_refinement_cycles; ++cycle) {
    ...
    VectorTools::project(space_dof_handler, constraints,
                         QGauss<dim>(space_fe.degree + 1), InitialValues<dim>(),
                         initial_solution);
    ...
    for (unsigned int k = 0; k < slabs.size(); ++k) {
      ...
      // evaluate solution at end of slab, i.e. on time inteval end of the last
      // time interval, as initial value for next slab
      FEValues<1> time_fe_values(slabs[k]->time_fe,
                                 Quadrature<1>({Point<1>(1.)}), update_values);
      ...
      for (unsigned int i = 0; i < space_dof_handler.n_dofs(); ++i)
        for (const unsigned int ii : time_fe_values.dof_indices())
          initial_solution(i) +=
              solution(i + local_dof_indices[ii] * space_dof_handler.n_dofs()) *
              time_fe_values.shape_value(ii, 0);
...
int main(int argc, char** argv) {
  try {
    ...
    unsigned int s = 1; //  spatial FE degree
    unsigned int r = 1; // temporal FE degree
    ...
    if (argc > 1)
    {
	    ...
	    {
		    if (std::string(argv[i]) == std::string("-r"))
		    {
			    r = std::stoi(argv[i+1]);
			    i++;
		    }
    ...
    // run the simulation
    SpaceTime<1> space_time_problem(s,             // s ->  spatial FE degree
                                    {r, r, r, r},  // r -> temporal FE degree
