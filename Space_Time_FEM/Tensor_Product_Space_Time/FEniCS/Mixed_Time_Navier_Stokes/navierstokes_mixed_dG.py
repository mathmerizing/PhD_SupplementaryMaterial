...
parser = argparse.ArgumentParser(description="Run a dG(r_v, r_p) simulation of Navier-Stokes.")
parser.add_argument("--r_v", type=int, default=1, help="polynomial degree of velocity space")
parser.add_argument("--r_p", type=int, default=0, help="polynomial degree of pressure space")
...
if not os.path.exists(f"results_mixed_dG/{PROBLEM}/{TIME_DG_QUADRATURE}_rv_{r_v}_rp_{r_p}_dt_{slab_size}"):
    os.makedirs(f"results_mixed_dG/{PROBLEM}/{TIME_DG_QUADRATURE}_rv_{r_v}_rp_{r_p}_dt_{slab_size}")

# configure logging
logging.basicConfig(filename=f"results_mixed_dG/{PROBLEM}/{TIME_DG_QUADRATURE}_rv_{r_v}_rp_{r_p}_dt_{slab_size}/output.log", level=logging.DEBUG)
...
logging.info(f"CONFIG: problem = {PROBLEM}, s = ({s_v}/{s_p}), r = ({r_v}/{r_p}) ({TIME_DG_QUADRATURE}), slab_size = {slab_size}")
...
xdmffile_v = XDMFFile(f"results_mixed_dG/{PROBLEM}/{TIME_DG_QUADRATURE}_rv_{r_v}_rp_{r_p}_dt_{slab_size}/velocity.xdmf")
xdmffile_p = XDMFFile(f"results_mixed_dG/{PROBLEM}/{TIME_DG_QUADRATURE}_rv_{r_v}_rp_{r_p}_dt_{slab_size}/pressure.xdmf")
...
for k, slab in enumerate(slabs):
    ...
    for time_element in Time["v"].mesh:
        for t_q in [time_element[0]] + [tq for (tq, _) in Time_high.quadrature_fine[time_element]] + [time_element[1]]:
            Uq = np.concatenate((
...
np.save(f"results_mixed_dG/{PROBLEM}/{TIME_DG_QUADRATURE}_rv_{r_v}_rp_{r_p}_dt_{slab_size}/times_draglift.npy", times_draglift)
np.save(f"results_mixed_dG/{PROBLEM}/{TIME_DG_QUADRATURE}_rv_{r_v}_rp_{r_p}_dt_{slab_size}/drag_values.npy", drag_values)
np.save(f"results_mixed_dG/{PROBLEM}/{TIME_DG_QUADRATURE}_rv_{r_v}_rp_{r_p}_dt_{slab_size}/lift_values.npy", lift_values)

#save plots as png
plt.title("Drag")
plt.plot(times_draglift, drag_values)
plt.savefig(f"results_mixed_dG/{PROBLEM}/{TIME_DG_QUADRATURE}_rv_{r_v}_rp_{r_p}_dt_{slab_size}/drag.png")
# clear plot
plt.clf()

plt.title("Lift")
plt.plot(times_draglift, lift_values)
plt.savefig(f"results_mixed_dG/{PROBLEM}/{TIME_DG_QUADRATURE}_rv_{r_v}_rp_{r_p}_dt_{slab_size}/lift.png")
